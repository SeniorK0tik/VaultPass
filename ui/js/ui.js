...
/**
 * Ждёт, пока браузер перерисует экран.
 *
 * Нужно перед долгой синхронной работой в ядре — Argon2id на разблокировке
 * занимает от четверти секунды до секунды, и надпись «Открываем…» должна
 * успеть появиться до того, как поток встанет.
 *
 * Кадра можно и не дождаться: пока окно скрыто или свёрнуто, браузер их не
 * рисует вовсе, и одинокий `requestAnimationFrame` не придёт никогда — кнопка
 * останется в состоянии «Проверяем…» навсегда. Поэтому рядом стоит таймер:
 * что сработает первым, то и продолжит работу.
 */
export function nextPaint(fallbackMs = 40) {
